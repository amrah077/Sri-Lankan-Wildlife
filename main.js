...
function fetchAndStoreJSON(JSONFile, storageKey) {   //storagekey is a placeholder to store the loaded data in server.
    fetch (JSONFile) //data is fetched(request) from the particular URL(i have used functions below to retriieve the correct data for each webpage.)
        .then(response => {
            ...
            return response.json(); //if response is okay, the JSON data is parsed data.
        })
...
//The LoggedInStatus function checks the user's login status from the session storage and updates the display of various elements on the web page accordingly. It shows or hides buttons based on whether the user is logged in.
//if the user is logged in, they wll remain logged in even if they move between different pages.
//here, loggedin is the key where true or false will be stored.
function LoggedInStatus(){
    //JSON.parse(loggedIn) converts the string "true" or "false" into the corresponding JavaScript boolean true or false.
    let loggedIn = sessionStorage.getItem('LoggedIn'); //using 'let' here cuz variable value will be crazy. SETTING THIS STRING VALUE TO THE SESSION STORAGE(JSON only stores strings).
...
function LoadContent() {  //This function is responsible for loading the content onto the webpage from the local storage based on the current page. 
    const storedData = JSON.parse(localStorage.getItem('websiteData'));  //JSON to JS objects. Retrieving dats from the local storage placeholder for pageData(websiteData).
    ...
    if (storedData) {  //checking if there is any data in the local storage.
        if (storedDataKey === currentPage) {  //checks if the key retrieved from the local storage matches the name of the current page.
            const pageData = storedData[storedDataKey];  //all the values(content)from the storedData is assigned to the relevant key . Retrieves the data for the current page from storedData and stores it in pageData.
            ...
            for (let i = 1; i <= Object.keys(pageData).length - 1; i++) {  
                ...
                if (section) {  //if that section exists....
                    ...
                    if (sectionTitleElement) {   //Checking if the section exists.
                        sectionTitleElement.textContent = section.title;  //When you use .textContent to set the text content of an element, it replaces any existing HTML or text inside that element with the new text string you provide.
                    }
...
function getCurrentPage() {
    ...
    const url = window.location.href;  //retrieves the full URL of the current page.
...
function makeEditable(elementId) {
    ...
    if (element) {  //if element exists...
        ...
        element.parentNode.replaceChild(editableElement, element); //(newchild, oldchild)
    }
}
...
function revertEdits(elementId) { //to be able to revert the edited content back to original HTML FORMAT.
...
function subscriptions () {
    ...
    let fullName = fullNameInput.value.trim(); //removing any whitespaces before or after name and email
    ...
    let subscriptions = JSON.parse(localStorage.getItem('newsletterSubscriptions')) || []; // retrieves the current list of subscriptions from local storage. It is parsed from JSON to a JavaScript object/array. If there are no subscriptions, JSON.parse returns null, so || [] ensures subscriptions is initialized as an empty array
    subscriptions.push({ fullName: fullName, email: email });
    //Adds the new subscription (an object with fullName and email properties) to the array.
...
function isEmailValid(email) {
    ...
    //the initial ^ represents first string position. the $ sign at end represents last string position of email.
    //[^\s@]+: Matches one or more characters that are not whitespace (\s) and not @. This ensures that the local part of the email (before the @ symbol) is not empty and doesn't contain spaces or the @ symbol. 
    return emailPattern.test(email);
    //The test method returns true if the email matches the pattern and false if it doesn't.
} 
